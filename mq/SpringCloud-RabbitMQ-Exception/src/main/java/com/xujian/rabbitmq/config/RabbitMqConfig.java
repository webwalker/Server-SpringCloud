package com.xujian.rabbitmq.config;
...
public class RabbitMqConfig {
    ...
    public Queue retryDirectQueue() {
        ...
        // 将消息重新投递到exchange中
        args.put(DEAD_LETTER_EXCHANGE_KEY, "scm3.materials");
     ...
     */
    @Bean("material@user2")
    public Queue userDirectQueue2() {
        return QueueBuilder.durable("material@user2").build();
    }

    @Bean
    public Binding userDirectBinding2(@Qualifier("material@user2") Queue queue,
                                      @Qualifier("scm3.materials2") Exchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with("direct_rounting_key").noargs();
    }

    @Bean("scm3.materials2")
    public Exchange directExchange2() {
        return ExchangeBuilder.directExchange("scm3.materials2").durable(true).build();
    }
}
