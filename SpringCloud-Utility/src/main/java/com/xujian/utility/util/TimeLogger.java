 ...
 *
 * <pre>
 *     TimeLogger timings = new TimeLogger(TAG, "methodA");
 *     // ... do some work A ...
...
public class TimeLogger {
    ...
    /**
     * Create and initialize a TimeLogger object that will log using
    ...
    /**
     * Clear and initialize a TimeLogger object that will log using
     * the specific tag. If the Log.isLoggable is not enabled to at
     * least the Log.VERBOSE level for that tag at creation time then
     * the addSplit and dumpToLog call will do nothing.
     * @param tag the log tag to use while logging the timings
...
//        mDisabled = !Log.isLoggable(mTag, Log.VERBOSE);
    ...
    public void dumpToLog() {
        if (!debug) return;
        logger.debug(mTag, mLabel + ": begin");
        ...
        for (int i = 1; i < mSplits.size(); i++) {
            ...
            logger.debug(mTag, mLabel + ":      " + (now - prev) + " ms, " + splitLabel);
        }
        logger.debug(mTag, mLabel + ": end, " + (now - first) + " ms");
    }
}
